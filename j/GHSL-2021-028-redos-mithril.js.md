<header class="post-header d-block mb-6">
      <div class="date text-mono f5 my-3">August 31, 2021</div>
      <h1 class="my-2 h00-mktg lh-condensed">GHSL-2021-028: ReDoS (Regular Expression Denial of Service) in mithril.js</h1>
...
<ul>
  <li>2021-01-21: Emailed report to contact@isiahmeadows.com</li>
  <li>2021-02-25: No response so far, so replied to the original email asking for an update.</li>
  <li>2021-02-25: Received a reply from Claudia Meadows (contact@isiahmeadows.com).</li>
  <li>2021-02-25: Pierre-Yves Gérardy has implemented a fix and created a <a href="https://github.com/MithrilJS/mithril.js/security/advisories/GHSA-c4vg-fvvw-q392">draft security advisory</a>.</li>
  <li>2021-04-13: I ask if the <a href="https://github.com/MithrilJS/mithril.js/security/advisories/GHSA-c4vg-fvvw-q392">draft security advisory</a> is ready to publish. Claudia Meadows responds that they will publish it in a few days time.</li>
  <li>2021-08-21: I ask again for the <a href="https://github.com/MithrilJS/mithril.js/security/advisories/GHSA-c4vg-fvvw-q392">draft security advisory</a> to be published.</li>
  <li>2021-08-31: Published as per our <a href="https://securitylab.github.com/advisories/#policy">policy</a>.</li>
</ul>
...
<p>mithril.js is a general purpose DOM manipulation libray.
It includes a CSS selector parser, that is vulnerable to ReDoS.
The attack requires that an attacker can inject a string into a CSS selector at runtime.
It is quite common to be able to inject into jQuery CSS selectors, so the attack seems realistic.
(Old versions of jQuery had a similar vulnerability, which is fixed now).</p>
...
<a href="https://lgtm.com/projects/g/MithrilJS/mithril.js/snapshot/5ba0bfd1ff044bc28c5ce1f2317d48fe0114a4db/files/render/hyperscript.js?sort=name&amp;dir=ASC&amp;mode=heatmap#L7">Link to query result</a>.</p>

<p>mithril.js is a browser library.
The PoC therefore invokes the CSS parser directly.</p>

<h3 id="poc">PoC</h3>
<ul>
  <li><code class="language-plaintext highlighter-rouge">npm install mithril</code></li>
  <li>Run the below with <code class="language-plaintext highlighter-rouge">node</code>
```JavaScript
var hyperscript = require(‘mithril/render/hyperscript’);</li>
</ul>
...
<p>You can contact the GHSL team at <code class="language-plaintext highlighter-rouge">securitylab@github.com</code>, please include a reference to <code class="l